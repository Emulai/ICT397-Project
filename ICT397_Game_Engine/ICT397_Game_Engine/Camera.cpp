...
void Camera::SetPlains(const int & moveX, const int & moveZ)
{
    ...
    for (int i = 0;  i < m_No_Plains; i++)
    {
        ...
        if ((m_x <= m_Plain.GetXend(i)) && (m_x >= m_Plain.GetXstart(i)) &&
            (m_y <= m_Plain.GetYend(i) + 100) && (m_y >= m_Plain.GetYstart(i) - 100) &&
            (m_z <= m_Plain.GetZend(i)) && (m_z >= m_Plain.GetZstart(i)))
        {
            // if flat plain
...
void Camera::MoveUD()
{
	if (m_CollisionDetectionOn)
	{
		GLdouble startY = m_y + m_deltaMoveUD * (m_lookYY) * m_gravSpeed * 5.0;

		if (!(m_colDetect.Collide(m_x + m_lookXX, startY + m_lookYY, m_z + m_lookZZ)))
		{
			m_y += m_deltaMoveUD * (m_lookYY) * m_gravSpeed;
			callGLLookAt();
		}
	}
	else
